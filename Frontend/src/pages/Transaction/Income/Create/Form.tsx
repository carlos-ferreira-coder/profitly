...
const Form = () => {
  ...
  // Income schema
  const schema = incomeCreateSchema
  ...
  // Create income in backend
  const createIncome = async (data: SchemaProps) => {
    setStatus('request')
    setAlertErrors(null)
    setAlertSuccesses(null)

    try {
      const response = await axios.post('/transaction/income/create', data, {
